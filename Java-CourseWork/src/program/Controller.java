...
public class Controller
{
    ...
    public void initTextFields()
    {
        ...
        txtOrdering.setTextFormatter(new TextFormatter<Number>(change -> numberFilter(change)));
        txtOrdering.setOnAction(event -> validateNumberField(txtOrdering, 1, 1000, 10));
        txtOrdering.focusedProperty().addListener(new ChangeListener<Boolean>() {
            @Override
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                validateNumberField(txtOrdering, 1, 1000, 10);
            }
        });
    }
    ...
    private void initSystemResources()
    {
        try
        {
            ...
            Configuration.setOrderingCount(Integer.parseInt(txtOrdering.getText()));
        }
